...
public class KissModClient implements ClientModInitializer {
        ...
        UseEntityCallback.EVENT.register((player, world, hand, entity, hitResult) -> {
            ...
            if (player.isSneaking()&& entity != null) {
                if (target != null) {
                    ...
                    ClientPlayNetworking.send(KissC2SPacket.PACKET_ID, packet.write());
    ...
    private void registerClientNetworkReceiver() {
        ClientPlayNetworking.registerGlobalReceiver(KissS2CPacket.PACKET_ID, (client, handler, buf, responseSender) -> {
            KissS2CPacket packet = new KissS2CPacket(buf);
            client.execute(() -> {
                ...
                if (world != null) {
                    for (Entity entity : world.getEntities()) {
                        if (entity.getUuid().equals(packet.getPattedEntityUuid())) {
                            if (MinecraftClient.getInstance().player != null && !MinecraftClient.getInstance().player.getUuid().equals(packet.getWhoPattedUuid())){
